...
(function (VG) {
  ...
  function DashboardQuickActionBar({ actions, can, accent, embedded }) {
  ...
  VG._dashboardChromeListeners = VG._dashboardChromeListeners || [];
  VG.setDashboardChrome = function (payload) {
    VG._dashboardChrome = payload || null;
    VG._dashboardChromeListeners.forEach((fn) => { try { fn(); } catch (e) {} });
  };
  VG.onDashboardChromeChange = function (fn) {
    VG._dashboardChromeListeners.push(fn);
    return () => { VG._dashboardChromeListeners = VG._dashboardChromeListeners.filter((f) => f !== fn); };
  };
  VG.DashboardQuickActionBar = DashboardQuickActionBar;
  ...
  function ModuleDashboard({ modId, mod, roleKey, can, go }) {
    ...
    useEffect(() => {
      if (!VG.setDashboardChrome) return;
      if (!cfg) {
        VG.setDashboardChrome(null);
        return;
      }
      VG.setDashboardChrome({
        modId,
        subtitle: cfg.subtitle || (mod && mod.tagline) || "",
        actions: cfg.quickActions || [],
        can,
        accent: (mod && mod.accent) || "var(--accent)",
      });
      return () => VG.setDashboardChrome(null);
    }, [modId, cfg, can, mod]);
